...
for mID, aMatch in matchD.items():
    totScore = 0
    for pID in range(55,66):
        try:
            pOverall = int(playerD[aMatch[pID]][8])
            if pOverall > 200:
                totScore += 45
            else:
                totScore += pOverall
            ...
            totScore += 45
    aMatch[55] = str(totScore/11)
    ## store the overall score at home_player_1

    totScore = 0
    for pID in range(66,77):
        try:
            pOverall = int(playerD[aMatch[pID]][8])
            if pOverall > 200:
                totScore += 45
            ...
            totScore += 45
    aMatch[56] = str(totScore/11)
    ## store the overall score at home_player_2
...
fv_cols.append("home_player_overall")
fv_cols.append("away_player_overall")
...
for key, val in fv.items():
    addPlayers = matchesWithKey[key]
    tempL = val[1].split('_')
    homeAttr = teamOverall_name[teamNamesRev[tempL[0]]]
    awayAttr = teamOverall_name[teamNamesRev[tempL[1]]]
    fv[key] += [addPlayers[55]] + [addPlayers[56]] + [str(homeAttr)] + [str(awayAttr)]

import csv

enhancedDataFile = open('Data/final_processed_3.csv', mode='w')
